class AppAssets {
  static const String _basePath = "assets/images/";

  // Onboarding Images
  static const String onboarding1 = "${_basePath}onboarding1.png";
  static const String onboarding2 = "${_basePath}onboarding2.png";
  static const String onboarding3 = "${_basePath}onboarding3.png";

  // Auth Images
  static const String signup = "${_basePath}signup.png";
  static const String signin = "${_basePath}signin.png";

  // Home Page Illustrations
  static const String taskSketching = "${_basePath}taskSketching.png";
  static const String taskPlan = "${_basePath}taskPlan.png";

  // Icons
  static const String user = "${_basePath}User.png";
  static const String lock = "${_basePath}Lock.png";
  static const String hide = "${_basePath}Hide.png";
  static const String notificationBell = "${_basePath}notificationBell.png";

  // Frames / Extra assets
  static const String frame1 = "${_basePath}Frame1.png";
  static const String frame4 = "${_basePath}Frame4.png";
  static const String frame5 = "${_basePath}Frame5.png";
  static const String rewaed = "${_basePath}Reward.png";
  static const String time = "${_basePath}Time.png";

  // Bottom Nav Bar
  static const String date = "${_basePath}Date.png";

  // Profile
  static const String profilePic = "${_basePath}ProfilePic.png";
  static const String edit = "${_basePath}Edit.png";
  static const String logout = "${_basePath}LogOut.png";
  static const String theme = "${_basePath}Theme.png";
  static const String secure = "${_basePath}Secure.png";
  static const String help = "${_basePath}Help.png";
}
